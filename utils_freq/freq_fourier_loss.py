...
def calc_fft(image):
    ...
    out_fft = torch.fft.fft2(image, dim=(-2, -1))
    fft = torch.stack((out_fft.real, out_fft.imag), -1)
...
def fft_L1_loss_mask(fake_image, real_image, mask):
    ...
    # print("fake_fft={}".format(fake_fft.shape))
    # print("real_fft={}".format(real_fft.shape))
