...
from .hDCE import PatchHDCELoss, PatchHDCELoss_Y
...
from utils_freq.freq_pixel_loss import find_fake_freq, get_gaussian_kernel, find_high_freq
...
class CUTModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_GAN', 'D_real', 'D_fake', 'G', 'fdt']
    ...
    def compute_G_loss(self):
        ...
        gauss_kernel = get_gaussian_kernel(11).cuda()
        mask_high, _ = decide_circle(r=80, N=1)  # L=128
        mask_high = mask_high.cuda()

        idtB_recon = self.idt_B
        realB = self.real_B
        real = self.real_A
        ...
        if self.opt.dce_idt and self.opt.lambda_HDCE > 0.0:
            ...
            self.loss_HDCE_Y = self.calculate_HDCE_Y_loss(real_B_pool, idt_B_pool, weight_idt)

            x_realB_freq, x_realB_high_freq = find_fake_freq(realB, gauss_kernel)
            x_idtB_recon, _ = find_fake_freq(idtB_recon, gauss_kernel)
            loss_rec_blur = F.l1_loss(x_idtB_recon, x_realB_freq)
            loss_recon_fft = fft_L1_loss_color(idtB_recon, realB)

            loss_HDCE_both = (self.loss_HDCE + self.loss_HDCE_Y) * 0.5 + loss_recon_fft + loss_rec_blur
        else:
            loss_HDCE_both = self.loss_HDCE

        x_fake_high_freq = find_high_freq(fake, gauss_kernel)
        x_real_high_freq = find_high_freq(real, gauss_kernel)

        loss_fake_blur = F.l1_loss(x_fake_high_freq, x_real_high_freq)
        loss_fake_high_fft = fft_L1_loss_mask(fake, real, mask_high)

        self.loss_fdt = loss_fake_blur + loss_fake_high_fft



        self.loss_G = self.loss_G_GAN + loss_HDCE_both + self.loss_SRC + self.loss_fdt
        return self.loss_G
    ...
    def calculate_HDCE_Y_loss(self, src, tgt, weight=None):
        ...
        total_HDCE_loss_Y = 0.0
        for f_q, f_k, crit, nce_layer, w in zip(feat_q_pool, feat_k_pool, self.criterionHDCE_Y, self.nce_layers, weight):
            if self.opt.no_Hneg:
                w = None
            loss = crit(f_q, f_k, w) * self.opt.lambda_HDCE
            total_HDCE_loss_Y += loss.mean()

        return total_HDCE_loss_Y / n_layers
